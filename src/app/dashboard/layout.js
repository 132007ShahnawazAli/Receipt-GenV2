...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[var(--background)] text-[var(--primary-text)]">
      ...
      <div
        className={`fixed inset-y-0 left-0 z-30 w-64 transform bg-(--background) border-r border-zinc-800 transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 ${
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="p-4 border-t border-zinc-800 space-y-2">
            {/* Development testing button - easy to remove */}
            {/* {process.env.NODE_ENV === "development" && (
              <button
                onClick={resetOnboardingData}
                className="w-full px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 transition-colors cursor-pointer text-white flex items-center justify-center gap-2"
              >
                <RefreshCw size={16} />
